...
# Read the file and store the contents of the file in a string.
...
# most used words in the string.
# The function will return the list of 10 most used words.
def most_used(words):
    ...
    # that each tuple is appended to
    for key, val in d_words.items():
        tup = (val, key)
        list.append(tup)
    # sorted so that it is biggest to smallest
    sort = sorted(list, reverse=True)
    # then is put into a list with only the words and not the amount of the words
